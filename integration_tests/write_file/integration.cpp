...
TEST(WriteFile, WriteFile){
 ...
 //SETUP -----------------------
    ...
    GapBuffer gbuffer;
    FileManager filemanager(test_file_path);
    ...
    std::string file_text = sstream_text.str();

    ASSERT_EQ(file_text, expected_file_text);
}